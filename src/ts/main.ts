const mainButton: HTMLImageElement = document.getElementById(
  "button",
) as HTMLImageElement,
...
let toStore = (key: string, value: any) =>
  localStorage.setItem(key, JSON.stringify(value));
let fromStore = (key: any) => JSON.parse(localStorage.getItem(key) as string);
let person = { image: "", mode: "", favColor: "", tasks: [] };

let updateStorage = (
  image: any = person.image,
  mode: string = person.mode,
  favColor: string = person.favColor,
  tasks: any = person.tasks,
) => {
  person.image = image;
...
userImage.addEventListener("change", (event) => {
  ...
  reader.addEventListener("load", () => {
    updateStorage(
      reader.result,
      fromStore("person").mode,
      fromStore("person").favColor,
      fromStore("person").tasks,
    );
...
const taskIcon: HTMLInputElement = document.getElementById(
  "task-icon",
) as HTMLInputElement,
  taskContent: HTMLInputElement = document.getElementById(
    "task-content",
  ) as HTMLInputElement,
  inputTaskColor: HTMLInputElement = document.querySelector(
...
document.querySelector(".input-box")?.addEventListener("submit", (e) => {
  if (taskContent.value !== "") {
    ...
    tasks.push(
      new Task(taskIcon.value, tasks.length, taskContent.value, false),
    );
...
statusList[0].addEventListener("click", () =>
  renderTasks(fromStore("person").tasks),
);
statusList[1].addEventListener("click", () =>
  renderTasks(
    fromStore("person").tasks.filter((ele: any) => (!ele.status ? ele : "")),
  ),
);
statusList[2].addEventListener("click", () =>
  renderTasks(
    fromStore("person").tasks.filter((ele: any) => (ele.status ? ele : "")),
  ),
);
...
if (fromStore("person")) {
  ...
  if (fromStore("person").favColor) {
    document
      .querySelector("html")
      ?.style.setProperty("--fav-color", fromStore("person").favColor);
  }
